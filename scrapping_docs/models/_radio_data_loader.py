"""
Générateur de diagramme de bandes de fréquences radio — POO
Logique de tracé : coordonnées réelles en MHz sur l'axe X.

Règles de rendu (fidèles au code original) :
  - 1 service  → rectangle pleine hauteur
  - 2 services → division verticale 50/50
  - >2 services → service[0] pleine hauteur + bandes fines h=0.2 pour services[1:]

Anti-chevauchement labels : zig-zag sur 6 niveaux + skip_every adaptatif.
Largeur figure : automatique (max 600 pouces), calculée sur l'étendue MHz.
"""
...
def readable_text_color(bg_color) -> str:
    return "black" if luminance(bg_color) > 0.6 else "white"
...
class ServiceColorPalette:
    ...
    def __init__(self, services: list[str]):
        colors = generate_distinct_colors(len(services))
...
# ─────────────────────────────────────────────
# Classe : labels anti-chevauchement
# ─────────────────────────────────────────────
